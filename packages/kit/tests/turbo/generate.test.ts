...
import { generateTurboConfig, type TurboConfig } from "../../src/turbo.js";
...
describe("generateTurboConfig errors", () => {
  ...
  it("throws on invalid JSON", () => {
    ...
    expect(() => generateTurboConfig(baseConfig(), { cwd })).toThrow(/not valid JSON/);
  });
  ...
  it("throws when package.json has no name", () => {
    ...
    expect(() => generateTurboConfig(baseConfig(), { cwd })).toThrow(/no "name" field/);
  });
});
